...
export default function index() {
  ...
  const submitHandlerTransaksi = async (values) => {
    ...
    if (res.status === "success") {
      ...
      setTimeout(() => {
        reset(), setStatus(""), setMessage(""), setLoading(false), navigate("/dashboard/kasir/transaksi");
      }, 500);
      return;
    }
  ...
  return (
    <Container>
      <Text cursor={"pointer"} onClick={() => navigate("/dashboard/kasir/transaksi")} mb={4}>{`<-- Kembali Ke Transaksi`}</Text>
      ...
      <Grid templateColumns={{ md: "repeat(2, 1fr)", lg: "repeat(3, 1fr)" }} gap={10} my={6}>
        <GridItem>
      ...
      <Flex flexDir={"column"}>
        ...
        {kolomMenu.map((row, indexRow) => (
          <Flex w={"full"} gap={10} my={6} alignItems={"flex-end"} key={indexRow}>
            <Flex direction="column">
              <Text fontSize={"sm"} fontFamily={"Poppins"}>
                Nama Menu
              </Text>
